...
# Documentation on faker can be found here - https://faker.readthedocs.io/en/master/providers/baseprovider.html
def test_fill_out_form_with_faker(page):
    ...
    page.fill("input[name='24emailadr']", fake.bothify(text='???????????@gmail.com'))   #utilizes bothify which is ? for alpha chars # for digits
    page.fill("input[name='25web_site']", fake.bothify(text='www.??????????????????-####????.com'))

    page.fill("input[name='30_user_id']", fake.bothify(text='?#?#?#?#?#?#?#?#?#?#?'))
    ...
    page.fill("input[name='45ccissuer']", fake.company()) #Card issuing bank
    page.fill("input[name='46cccstsvc']", fake.phone_number()) #Customer Service Phone Number
    page.fill("input[name='60pers_sex']", fake.bothify(letters='MF', text='?')) #selects a random letter but only from list of M and F
    page.fill("input[name='61pers_ssn']", fake.ssn())
    page.fill("input[name='62driv_lic']", fake.bothify(text='#####################'))
    page.fill("input[name='66pers_age']", str(fake.pydecimal(min_value=5, max_value=100))) #generates a random digit must convert to string
    ...
    #Credit Card type
    cardType = fake.word(ext_word_list=['amex', 'diners', 'discover', 'mastercard', 'visa'])
    #creditCardType = fake.credit_card_provider(card_type=cardType) #Would generate a random credit card based upon the types above but not used here
    ...
    #removes leading zero if one is present in month as its not present in value of options
